...
pot
```
## Vývoj všetkých ochorene, color = Column)) + geom_line() + theme_minimal()
```
...
df_long <- tidyr::pivot_longer(df, cols = c(y1, y2, y3), names_to = "Column", values_to = "Value")

# Plot the data using ggplot2
ggplot(df_long, aes(x = x, y = Value, color = Column)) +
  geom_line() +
  labs(x = "X", y = "Y") +
  theme_minimal()
```

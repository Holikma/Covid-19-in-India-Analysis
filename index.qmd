...
max(India_Vaccination_Dataset$Celkom_očkovaný)
max(ALL_Testing_Dataset$Počet_vakcinácií)
```

```{r}
nc <- st_read("India-map/India_State_Boundary.shp", quiet=TRUE)[-13,]
nc <- nc[order(nc$State_Name),]
nc$State_Name <- ALL_Testing_Dataset$Štát
rename(nc, "Štát" = "State_Name")
```


```{r}
ggplot(nc) + 
    geom_sf(aes(fill=ALL_Testing_Dataset$Potvrdené_Prípady)) + 
